      ...
      >
        <el-form ref="form" label-width="80px" label-position="left">
          ...
          <el-form-item label="Type" label-width="70px">
            <el-select v-model="editSoftware.menuName" placeholder="请选择Menu" @change="changeSelect">
              <el-option
                v-for="(item, index) in menu"
                :key="index"
                :label="item.name"
                :value="item.id"
                @click.native="selectClick(item.id,item.name)"
              ></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="Note" label-width="70px">
...
export default {
  ...
  methods: {
    ...
    cancel() {
      ...
      this.editSoftware.id = "";
      this.editSoftware.type = "";
      this.name = "";
      this.version = "";
      this.note = "";
      this.type = 1;
